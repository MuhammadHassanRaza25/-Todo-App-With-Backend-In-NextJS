...
export default function Header(){
  ...
  const toggleMenu = () => {
    ...
    return(
    <>
   <div className='header flex justify-between items-center p-3.5 py-3 px-7 bg-slate-800 border-b-2 border-cyan-400 text-3xl font-semibold w-full'>
      ...
      <span className="hidden md:flex gap-4">
        <Link href="https://github.com/MuhammadHassanRaza25" target="_blank">
          <button className='btn text-lg font-semibold hover:bg-white hover:text-black w-24 rounded-md p-1 bg-cyan-400 text-white'>Github</button>
        </Link>
        <Link href="https://www.linkedin.com/in/muhammad-hassan-raza-aab5402b7/" target="_blank">
          <button className='btn text-lg font-semibold hover:bg-white hover:text-blue-500 w-24 rounded-md p-1 bg-cyan-400 text-white'>Linkedin</button>
        </Link>
      </span>
      ...
      <button
        ...
        onClick={toggleMenu}
      >
        <div className={`w-6 h-1 bg-white transition-all duration-300 ${isMenuOpen ? 'transform rotate-45 translate-y-2' : ''}`} />
        <div className={`w-6 h-1 bg-white transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`} />
        <div className={`w-6 h-1 bg-white transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
      </button>

      {/* Mobile Menu (Hidden by default) */}
      <div className={`absolute top-16 right-0 w-full bg-slate-800 p-4 text-center transition-all duration-300 ${isMenuOpen ? 'block' : 'hidden'} md:hidden`}>
        <Link href="https://github.com/MuhammadHassanRaza25" target="_blank">
          <button className='btn mr-5 text-lg font-semibold hover:bg-white hover:text-black w-24 rounded-md p-1 bg-cyan-400 text-white mb-4'>Github</button>
        </Link>
        <Link href="https://www.linkedin.com/in/muhammad-hassan-raza-aab5402b7/" target="_blank">
          <button className='btn text-lg font-semibold hover:bg-white hover:text-blue-500 w-24 rounded-md p-1 bg-cyan-400 text-white'>Linkedin</button>
        </Link>
